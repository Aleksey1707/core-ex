...
- **Свод приложения-потребителя (`docs/rules/app/*.md`).** Второй ярус свода: нормы, общие для
  ...
  `make rules-check` проверяет оба яруса и падает на имени конкретного потребителя в `app/**`.
  ...
  разрешимость ссылок `deps/core/…`, скиллы с тремя файлами темы, импорты `20-agreements.md`
  и симлинк `CLAUDE.md` → `AGENTS.md`. Цель `rules-check` в `Makefile` приложения меняется на
  этот вызов.
