...
Почему: `Repo.Sc` фиксирует эталон по первому чтению, а `Version` проверяется на записи. Разнесённые load/save дают запись по устаревшей копии, немой пропуск `update` (`Sc.find(context, Entity, id) == entity`) и невидимую в одном месте границу транзакции.
