...
| Звено | Span | Связь |
|---|---|---|
| `<Aggregate>.Outbox.from_event/1` | нет; `traceparent` команды кладётся в `Record.headers` | — |
...
Родитель обработчика — create-span своего сообщения, а не пачка: иначе трейс команды
обрывался бы на поллере, а сообщения одной пачки склеились бы в один трейс.
