...
У event-sourced агрегата (`aggregate:`) case проверяет ещё полноту `evolve` — `19-testing.md`,
«Event-sourced агрегат». Фикстуры текущих тегов гоняет и `use Core.Es.ProjectionCase` — полноту
`project/1` и `clear/0` проекции (`19-testing.md`, «Проекции»).
