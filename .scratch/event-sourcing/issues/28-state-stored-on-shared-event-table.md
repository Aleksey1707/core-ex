...
- [x] `Core.Repo.Pg.Es` переименован в `Core.Repo.Pg.StateStored`; `event_repo:` заменена обязательной `event_codec:`.
- [x] `CompileError`: кодек без `type:`; Prim `__es_aggregate_id__/0` кодека не равен `id:`; `event:` у `outbox:`
...
- [x] Порядок записи в одном `Transact.run`: строка → дочерние строки → `outbox.from_events` → `Core.Es.Store.append`
      без непрерывности потока → `outbox_repo().append`; `Repo.Sc`, `shadow_copy?` и `check_events_for_change!` не
      меняются.
- [x] `:version_mismatch` на записи событий — код из `errors:`, ns — `behaviour:`, detail
      `%{aggregate_id, expected, actual}`.
- [x] Удалены `Core.Es.Event.Repo`, `Core.Es.Event.Repo.Pg`, `Core.Es.Event.Repo.Pg.Schema` вместе с
      `list_by_aggregate`, `count_by_aggregate`, `page_by_aggregate`, их тестами и ссылками в `Core.Config`, `Core.DAO`
      и `Repo.Pg.Children`.
- [x] `Entity` / `Child` из теста `Repo.Pg.Es` переехали в `test/support` (таблицы — в тестовых миграциях, кодек —
...
- [x] `CHANGELOG.md`, «Ломающие изменения контракта» — дополнить существующие пункты про `Es.Event.Repo.Pg.Schema`, а
...
- [x] `CHANGELOG.md`, «Изменения контракта макросов»: `Core.Repo.Pg.StateStored` с `event_codec:` и сверками.
- [x] Своды:
...
- [x] README: «Что предоставляет потребитель» без пункта про Ecto-тип jsonb, новые имена модулей; `description` skill
      `repos`. ADR не правятся.
- [x] `make` зелёный, включая `boundary-check` и `xref`.
