...
- [x] Возврат `{:ok, Pagination.Result.t(Es.Event)}` по `Pagination.Limit` / `Pagination.Offset`: порядок — по
      возрастанию `aggregate_version`, `count` — весь поток; тип агрегата и семейство событий — из кодека.
- [x] Условия `pg_snapshot_xmin` в запросе нет.
- [x] Строки грузятся фасадом `InCodec.load(Agg.Event, wire)`, апкаст действует; первая ошибка `load`
      (`:unknown_event_type`, `:invalid_envelope`) — `{:error, _}` на всю страницу через `Result.traverse`, без
      `raise`.
- [x] Пустой поток — страница с `count: 0`, `:not_found` нет; доступ не проверяется; `context` — последний из данных и
      не используется; `opts`, span и telemetry нет.
- [x] Тесты: страница и `count`; `offset` за концом потока; пустой поток; поток того же `aggregate_id` другого типа
      агрегата на страницу не попадает; событие с неизвестным тегом даёт ошибку всей страницы.
- [x] `13-repos.md`: H2 «Страница потока» — `page_stream/5`; MUST проверить права и существование агрегата через
