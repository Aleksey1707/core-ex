defmodule Blind.Account do
  @moduledoc """
  Корректный агрегат: команды qc-стиля (`%mod{} when mod in @on_existing`), две команды одной
  командой через `fold/3`, события с нагрузкой и без.
  """
  ...
  defmodule Cmd do
    ...
    # Команда без clause в decide/2 (сценарий A2).
    defmodule Orphan do
      use Core.Es.Cmd
  ...
  def decide(%Cmd.Open{name: name}, %__MODULE__{version: nil}),
    do: {:ok, [Event.Codec.draft(Event.Opened.Payload.new(name))]}
  ...
  def decide(%Cmd.Rename{name: name}, %__MODULE__{status: :open}),
    do: {:ok, [Event.Codec.draft(Event.Renamed.Payload.new(name))]}
