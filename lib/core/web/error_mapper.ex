defmodule Core.Web.ErrorMapper do
  ...
  Доменный `:not_found` своей строки не имеет и уходит 400 `:domain_error` по строке
  `kind: :domain`, а не 404. `:not_found` домен отдаёт и на ресурс из URI, и на запись, на
  которую ссылается тело команды, а по `%Error{}` маппер их не различит: 404 на существующий
  ресурс клиент прочтёт как «ресурса нет». Отказу домена на команде с `If-Match` 404 не
  нужен — по RFC 9110 §13.2.1 ответу достаточно не быть ни 2xx, ни 412. Приложение, чей
  домен разводит коды «ресурс не найден» и «ссылка не найдена», может отдать 404 своей
  клозой `map/1` перед делегированием.
