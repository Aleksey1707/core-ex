defmodule Core.Es.Projection do
  ...
  - иначе опрос строки чекпоинта с удвоением интервала от 10 до 100 мс; истёк `timeout` —
    прикладная `:projection_timeout` с `timeout` в detail. Проекция в повторе чекпоинт не двигает,
    и ожидание идёт до таймаута.
