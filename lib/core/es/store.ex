defmodule Core.Es.Store do
  ...
  `page_stream/5` отдаёт `Core.Pagination.Result` из событий потока одного агрегата по
  возрастанию версии; `count` — число событий всего потока. Условия видимости по `xid` в
  запросе нет: порядок внутри потока держит версия. Строки грузятся фасадом, апкаст действует;
  первая ошибка загрузки (`:unknown_event_type`, `:invalid_envelope`) — `{:error, _}` на всю
  страницу, событие не пропускается.
  ...
  Позиция последнего события потока — цель ожидания проекции (`Core.Es.Projection.await/4`) —
  ...
  Тип агрегата и семейство событий — из кодека `event_codec`. `context` не используется.
  Исходы — в `@moduledoc`.
  """
  @spec page_stream(
          module(),
