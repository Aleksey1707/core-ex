defmodule Core.Repo.Pg.StateStored do
  @moduledoc """
  ...
  Каждая запись идёт одной транзакцией: строка агрегата → дочерние строки →
  `outbox.from_events` → `Core.Es.Store.append` → `Core.Config.outbox_repo().append`.
  Дочерние строки пишет `Core.Repo.Pg.Children` — точечно, а не перезаписью всей коллекции:
  ...
  События пишутся в поток хранилища событий (`Core.Es.Store`) с типом агрегата из `type:`
  ...
  заголовки переопределённых функций, `id:` сверяется с кодеком.

  На компиляции `CompileError`, если `event_codec:` — не кодек событий с `type:`, Prim
  агрегата кодека (`__es_aggregate_id__/0`) не равен `id:`, событие `outbox:`
  (`__es_event__/0`) не равно семейству кодека, в `errors:` нет clause `:version_mismatch`.
  ...
  @label "Repo.Pg.StateStored"
  @children_label "#{@label}: children"
  @own_keys ~w(event_codec outbox children)a
  @required_callbacks ~w(insert update save)a

  @doc "Реализовать write-репозиторий state-stored агрегата через PostgreSQL."
  defmacro __using__(opts) do
    ...
    quote do
      ...
      @es_event_codec unquote(cfg.event_codec)
      ...
      @doc "Вставить агрегат с дочерними строками; записать события в хранилище и outbox."
      @impl true
      ...
      @doc "Обновить агрегат с дочерними строками; записать события в хранилище и outbox."
      @impl true
      ...
      @doc "Сохранить агрегат (insert или update) с записью событий."
      @impl true
      ...
      # `Core.Es.Store.append` отвергает занятую версию потока, а `optimistic_lock` на строке
      # агрегата не используется. Мутация, изменившая состояние, но не породившая события,
      # эту защиту обходит молча.
        ...
        if Core.Es.Events.empty?(entity.events) and not is_nil(baseline) and
          ...
          raise ArgumentError,
                "#{inspect(unquote(cfg.entity))}: состояние изменено без события — " <>
                  "запись обошла бы проверку конкурентности хранилища событий"
        end
      ...
      defp do_flush_events(events, context, opts) do
        with {:ok, records} <- @es_outbox.from_events(events),
             :ok <- Core.Es.Store.append(@es_event_codec, events, context, &version_mismatch/1) do
          Core.Config.outbox_repo().append(records, context, opts)
        end
      end

      defp version_mismatch(detail),
        do: @pg.errors.domain(@pg.module, :version_mismatch, detail)
    end
  end
  ...
  defp validate_opts!(opts) do
    ...
    Helper.Opts.validate!(
      opts,
      Core.Repo.Pg.required_keys() ++ ~w(to_entity to_model event_codec outbox entity id)a,
