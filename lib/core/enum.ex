defmodule Core.Enum do
  ...
  Обязательная опция `name:` и ровно один источник значений: `values:` (непустой список
  атомов без дублей) либо `codes:`. Оба сразу или ни одного — `CompileError`.
  Значение — голый атом (не `%Mod{value:}` как у Prim).

  `codes:` — `%{atom => code}` для enum, значения которого нумерует или именует внешний
  источник. Коды одного типа — все целые либо все непустые строки — и без повторов, иначе
  `CompileError`. `values/0` выводится из ключей карты и упорядочен по коду; дополнительно
  генерируются `codes/0`, `to_code/1`, `from_code/1` и `from_code!/1`.
  """
